    ...
    window.google.accounts.id.initialize({
      ...
      callback: (response) => {
        try {
          ...
          onLogin(user);
        } catch (err) { console.error("Error decoding Google credential", err); }
      },
      auto_select: false,
      cancel_on_tap_outside: true
    });

    window.google.accounts.id.renderButton(document.getElementById("gsi-button"), { theme: "outline", size: "large", width: "220" });
